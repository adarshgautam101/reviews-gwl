document.addEventListener('DOMContentLoaded', function () {
    ...
    const updateCardWidths = () => {
        ...
        // Calculate responsive cards count for BOTH vertical and horizontal
        let responsiveCards = cardsPerPage;
        if (window.innerWidth < 768) {
            responsiveCards = 1;
        } else if (window.innerWidth < 992) {
            responsiveCards = Math.min(cardsPerPage, 2);
        }
        // Ensure we don't exceed 6 or go below 1
        const cardsToShow = Math.max(1, Math.min(responsiveCards, 6));

        if (sliderDirection === 'vertical') {
            sliderBlock.style.setProperty('--card-width', '100%');
            sliderBlock.style.setProperty('--card-width-lg', '100%');
            sliderBlock.style.setProperty('--card-width-md', '100%');
            sliderBlock.style.setProperty('--card-width-sm', '100%');
            sliderBlock.style.setProperty('--card-width-xs', '100%');

            // Dynamic height calculation
            // Minimum height per card to ensure content is visible
            const minCardHeight = 250;
            const paddingAndArrows = 120;

            // Calculate required container height
            const requiredHeight = (minCardHeight * cardsToShow) + (gap * (cardsToShow - 1)) + paddingAndArrows;

            // Apply height to container
            if (elements.sliderContainer) {
                elements.sliderContainer.style.height = `${requiredHeight}px`;
            }

            sliderBlock.style.setProperty('--card-height', `${minCardHeight}px`);
            sliderBlock.style.setProperty('--cards-per-slide', cardsToShow);

        } else {
            // Horizontal Mode
            // Reset container height to fixed value (or remove inline style to let CSS handle it)
            if (elements.sliderContainer) {
                elements.sliderContainer.style.height = '500px'; // Restore default fixed height for horizontal
            }

            // Ensure card height is consistent with the fixed container height
            sliderBlock.style.setProperty('--card-height', '100%');
            sliderBlock.style.setProperty('--cards-per-slide', cardsToShow);
        }
    };
    ...
    function applySettingsToCss(settings) {
        ...
        setTimeout(() => {
            showCorrectLayout(); // Show container first so clientHeight is available
            updateCardWidths();
            applySliderSettings();
        }, 50);
    }
    ...
    function fetchAppSettings() {
        ...
        const shop = sliderBlock.dataset.shop;
        if (!apiUrl) {
            applySettingsToCss({
                starColor: '#FFD700',
        ...
        const fetchUrl = shop ? `${apiUrl}${apiUrl.includes('?') ? '&' : '?'}shop=${shop}` : apiUrl;
        fetch(fetchUrl)
            .then(response => {
                if (!response.ok) {
    ...
    function createReviewCard(review) {
        ...
        // Cards will always take 1 column in the 6-column grid to keep width fixed
    ...
    function showNextReviews() {
        if (displayType === 'slider' && currentIndex + cardsPerPage < reviews.length) {
            // Ensure we don't go past the last possible full page
            currentIndex = Math.min(currentIndex + cardsPerPage, reviews.length - cardsPerPage);
            renderSliderReviews();
            updateButtons();
    ...
    function showPrevReviews() {
        if (displayType === 'slider' && currentIndex > 0) {
            ...
            // Go to the last possible full page
